...
uint64_t bntree::search(QString key) {
    if (!this->tree->root) {
        return -1;
    }
...
void bntree::insert(QString key, QString val) {
...
void bntree::rec(QTreeWidget *tree, QTreeWidgetItem *p, node_t *r, bool b) {
    if (r == nullptr) return;

    QTreeWidgetItem *itm = new QTreeWidgetItem();
    if (b) itm->setBackground(0, Qt::gray);
    else itm->setBackground(0, Qt::red);
    itm->setText(0, r->data.key);
    itm->setText(1, r->data.value);
    p->addChild(itm);
...
void bntree::output(QTreeWidget *tree) {
    tree->clear();
        if (this->tree->root == nullptr) return;
        QTreeWidgetItem *item = new QTreeWidgetItem(tree);

        item->setText(0, this->tree->root->data.key);
        item->setText(1, this->tree->root->data.value);
        tree->addTopLevelItem(item);

        rec(tree, item, this->tree->root->left, true);
        rec(tree, item, this->tree->root->right, false);
...
QString bntree::findMax(node_t* p) {
    if (p == NULL)
        return NULL;

    QString res = p->data.key;
    QString left = findMax(p->left);
    QString right = findMax(p->right);
    if (left > res) res = left;
    if (right > res) res = right;
    return res;
}

node_t* bntree::getParent(node_t *p, QString key) {
    if (p == NULL) return NULL;
    else if (p->right->data.key == key || p->right->data.key == key) return p;
    else if (p->right->data.key > key) return getParent(p->left, key);
    else return getParent(p->right, key);
}

void bntree::removeMaxBranch() {
    //find max
    QString max = findMax(this->tree->root);

    //remove max branch
    node_t* parent = getParent(this->tree->root, max);
    if (parent->left->data.key == max) {
        parent->left = NULL;
    }
    else {
        parent->right = NULL;
